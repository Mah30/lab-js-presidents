...
function  countYearsInOffice(presidentsArr) {
  ...
  //return presidentsArr.reduce((totalYears, president) => {
    ...
    //const leftYear = president.leftOffice || new Date().getFullYear();
    //const yearsInOffice = leftYear - president.tookOffice; 
...
function sortPresidentsByBirthYear(presidentsArr) {
  ...
  const formatted = presidentsArr.map(president => ({
    name: president.name,
...
function countRepublicanPresidents(presidentsArr) {
  ...
  return presidentsArr.reduce((total, president) => {
    return total + (president.party === "Republican" ? 1 : 0);
  }, 0);
}
...
return presidentsArr.sort((a, b) => (a.name < b.name ? -1 : a.name > b.name ? 1 : 0));
